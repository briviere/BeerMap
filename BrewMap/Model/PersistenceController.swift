...
struct PersistenceController {
    ...
    static var preview: PersistenceController = {
        ...
        let restaurant = Brewery(context: viewContext)
    ...
    static var testData: [Brewery]? = {
        let fetchRequest = NSFetchRequest<NSFetchRequestResult>(entityName: "Brewery")
        return try? PersistenceController.preview.container.viewContext.fetch(
            fetchRequest) as? [Brewery]
    }()
    
    
    init(inMemory: Bool = false) {
        container = NSPersistentContainer(name: "BrewMap")
        if inMemory {
